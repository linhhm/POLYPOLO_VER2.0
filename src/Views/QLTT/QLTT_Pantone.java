 ...
 * @author X1
 */
public class QLTT_Pantone extends javax.swing.JFrame {
    ...
    public QLTT_Pantone() {
        ...
        setIcon();
        loadTable(spService.getListMauS());
    }
    ...
    //LOAD
    void loadTable(ArrayList<SP_MauSac> ls) {
        DefaultTableModel model = (DefaultTableModel) tblColor.getModel();
        model.setRowCount(0);
        for (SP_MauSac ms : ls) {
            model.addRow(new Object[]{
                ms.getIdMau(), ms.getTenMau()
            });
        }
    }
    
    //GETMODEL
    public SP_MauSac getModel(){
        int pos = tblColor.getSelectedRow();
        
        SP_MauSac ms = new SP_MauSac();
        ms.setIdMau((Integer) tblColor.getValueAt(pos, 0));
        ms.setTenMau(txtTenTT.getText().trim());
        return ms;
    }
    public SP_MauSac getUpdate(){
        SP_MauSac ms = new SP_MauSac();
        
        ms.setIdMau(Integer.valueOf(txtMaTT.getText()));
        ms.setTenMau(txtTenTT.getText().trim());
        return ms;
    }
    ...
    public Boolean validateThuocTinh(){
        ...
        v.isEmpty(txtTenTT, stb, "Tên màu bị trống!");
        if (stb.length()>0) {
    ...
    private void initComponents() {
        ...
        jLabel2.setText("THÔNG TIN MÀU SẮC");
    ...
    private void btnAdd_BMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnAdd_BMouseClicked
        //ADD 
        int result = JOptionPane.showConfirmDialog(this, "Bạn muốn thêm mới màu sắc?", "POLYPOLO xác nhận", JOptionPane.YES_NO_OPTION);   
        if (result == JOptionPane.YES_OPTION && validateThuocTinh()) {
            JOptionPane.showMessageDialog(this, spService.addColor(getModel()));
            loadTable(spService.getListMauS());
        }else{
            JOptionPane.showMessageDialog(this, "Đã hủy thao tác thêm mới màu sắc!", "POLYPOLO thông báo", 0);
        }
    ...
    private void btnUpdate_BMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnUpdate_BMouseClicked
        // UPDATE
        int selectedRow = tblColor.getSelectedRow();
        if (selectedRow == -1) {
            JOptionPane.showMessageDialog(this, "Vui lòng chọn màu sắc bạn muốn cập nhật!", "POLYPOLO thông báo", JOptionPane.WARNING_MESSAGE);
        } else {
            int result = JOptionPane.showConfirmDialog(this, "Bạn muốn cập nhật thuộc tính?", "POLYPOLO xác nhận", JOptionPane.YES_NO_OPTION);
            if (result == JOptionPane.YES_OPTION && validateThuocTinh()) {
                JOptionPane.showMessageDialog(this, spService.updateMauSac(getUpdate()));
                loadTable(spService.getListMauS());
            } else {
    ...
    private void btnHide_BMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnHide_BMouseClicked
        ...
        int selectedRow = tblColor.getSelectedRow();
        if (selectedRow == -1) {
            JOptionPane.showMessageDialog(this, "Vui lòng chọn màu sắc bạn muốn ẩn!", "POLYPOLO thông báo", JOptionPane.WARNING_MESSAGE);
        } else {
            int result = JOptionPane.showConfirmDialog(this, "Bạn muốn ẩn thuộc tính?", "POLYPOLO xác nhận", JOptionPane.YES_NO_OPTION);
            if (result == JOptionPane.YES_OPTION) {
                spService.hideMauSac(getModel());
    ...
    private void btnShow_BMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnShow_BMouseClicked
        // HIDDEN LIST
        loadTable(spService.getColorHide());
    ...
    private void btnUnhide_BMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnUnhide_BMouseClicked
        ...
        int selectedRow = tblColor.getSelectedRow();
        if (selectedRow == -1) {
            JOptionPane.showMessageDialog(this, "Vui lòng chọn màu sắc bạn muốn bỏ ẩn!", "POLYPOLO thông báo", JOptionPane.WARNING_MESSAGE);
        } else {
            int result = JOptionPane.showConfirmDialog(this, "Bạn muốn bỏ ẩn thuộc tính?", "POLYPOLO xác nhận", JOptionPane.YES_NO_OPTION);
            if (result == JOptionPane.YES_OPTION) {
                spService.unhideMauSac(getModel());
                loadTable(spService.getListMauS());
    ...
    private void tblColorMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblColorMouseClicked
        ...
        int selectedRow = tblColor.getSelectedRow();        
        if (selectedRow != -1) {
            Integer idBrand = Integer.valueOf(tblColor.getValueAt(selectedRow, 0).toString());
            String msName = tblColor.getValueAt(selectedRow,1).toString();
            txtMaTT.setText(idBrand.toString());
            txtTenTT.setText(msName);
        }
    ...
    private void btnSearchBrandMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnSearchBrandMouseClicked
        ...
        ArrayList<SP_MauSac> ls = spService.searchByNameColor(name);
        loadTable(ls);
        txtSearch.setText("");
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new QLTT_Pantone().setVisible(true);
            }
        });
    }
